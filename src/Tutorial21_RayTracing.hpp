...
private:
    ...
    // Esferas
    static constexpr int NumSpheres = 3;
    static constexpr int MaxSmallSpheres = 200;
    int                  m_shownSpheres    = 50;

    static constexpr int MaxSmallCubes = 200;
    int                  m_shownCubes  = 50;

    // Lista de esferas pequenas
    std::vector<std::string> m_SmallSphereNames;
    std::vector<float3>      m_SmallSpherePositions;

    // TODO: Colores
    float3 SphereColors[MaxSmallSpheres];

    // lista de cubos
    std::vector<std::string> m_SmallCubesNames;
    std::vector<float3>      m_SmallCubesPositions;

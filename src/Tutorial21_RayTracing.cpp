...
void Tutorial21_RayTracing::CreateProceduralBLAS()
{
    static_assert(sizeof(HLSL::BoxAttribs) % 16 == 0, "BoxAttribs must be aligned by 16 bytes");
     ...
     const HLSL::BoxAttribs Boxes[] = {
        ...
        // El tamano que van a tener las esferas pequenas
        HLSL::BoxAttribs{-0.5f, -0.5f, -0.5f, 0.5f, 0.5f, 0.5f},
     
        HLSL::BoxAttribs{-0.25f, -0.25f, -0.25f, 0.25f, 0.25f, 0.25f}};
    ...
    // Create box buffer
    {
        ...
        BuffDesc.ElementByteStride = sizeof(Boxes[1]);
...
void Tutorial21_RayTracing::UpdateTLAS()
{
    ...
    // TODO:
    static constexpr int NumInstances  = 1 + NumSpheres + MaxSmallSpheres + MaxSmallCubes;
    const int            InstanceCount = 1 + NumSpheres + m_shownSpheres + m_shownCubes;
    ...
    // Poblar con esferas grandes

    Instances[1].InstanceName = "Sphere Instance 1";
    Instances[1].CustomId     = 0;
    Instances[1].pBLAS        = m_pProceduralBLAS;
    Instances[1].Mask         = OPAQUE_GEOM_MASK;
    Instances[1].Transform.SetTranslation(-3.0f, -3.0f, -5.f);
    ...
    Instances[2].Transform.SetTranslation(3.0f, -3.0f, -5.f);
    ...
    Instances[3].Transform.SetTranslation(9.0f, -3.0f, -5.f);

    // Poblar con esferas pequenas
    static std::mt19937                          rng{std::random_device{}()};
    static std::uniform_real_distribution<float> distAngle(0.f, 2 * PI_F);
    static std::uniform_real_distribution<float> distUnit(0.f, 1.f);

    const float radius = 10.0f;
    const float3 center{3.0f, -3.0f, -5.0f};
    for (int i = 0; i < MaxSmallSpheres; ++i)
        {
            auto& inst        = Instances[4 + i];
            inst.InstanceName = m_SmallSphereNames[i].c_str();
            inst.pBLAS        = m_pProceduralBLAS;
            inst.Mask         = (i < m_shownSpheres) ? OPAQUE_GEOM_MASK : 0u;
            inst.CustomId     = 1;
            
            const auto& pos = m_SmallSpherePositions[i];
            inst.Transform.SetTranslation(pos.x, pos.y, pos.z);
            
        }

    // Poblar cubos pequenos
    static std::mt19937 rng2{std::random_device{}()};
    for (int i = 0; i < MaxSmallCubes; ++i)
        {
            auto& inst        = Instances[204 + i];
            inst.InstanceName = m_SmallCubesNames[i].c_str();
            inst.pBLAS        = m_pCubeBLAS;    
            inst.Mask         = (i < m_shownCubes) ? TRANSPARENT_GEOM_MASK : 0u;

            const auto& pos = m_SmallCubesPositions[i];
            inst.Transform.SetTranslation(pos.x, pos.y, pos.z);
    ...
    // TODO: Cambiar numero de esferaas
...
void Tutorial21_RayTracing::CreateSBT()
{
    ...
    for (int i = 0; i < MaxSmallSpheres; ++i)
    {
        const char* name = m_SmallSphereNames[i].c_str();
        m_pSBT->BindHitGroupForInstance(m_pTLAS, name, PRIMARY_RAY_INDEX, "SpherePrimaryHit");
    }

    for (int i = 0; i < MaxSmallCubes; ++i)
    {
        const char* name = m_SmallCubesNames[i].c_str();
        m_pSBT->BindHitGroupForInstance(m_pTLAS, name, PRIMARY_RAY_INDEX, "GlassPrimaryHit");
    }
...
void Tutorial21_RayTracing::Initialize(const SampleInitInfo& InitInfo)
{
    ...
    // Lista de nombres esferas pequenas
    m_SmallSphereNames.resize(MaxSmallSpheres);
    for (int i = 0; i < MaxSmallSpheres; ++i)
        m_SmallSphereNames[i] = "SmallSphere" + std::to_string(i);

    // Lista de posiciones de esferas pequenas
    const float                           radius = 10.0f;
    const float3                          center{3.0f, -3.0f, -5.0f};
    std::mt19937                          rng{std::random_device{}()};
    std::uniform_real_distribution<float> distAngle(0.f, 2 * PI_F);
    std::uniform_real_distribution<float> distUnit(0.f, 1.f);

    m_SmallSpherePositions.resize(MaxSmallSpheres);
    for (int i = 0; i < MaxSmallSpheres; ++i)
    {
        float teta                = distAngle(rng);
        float r                   = std::sqrt(distUnit(rng)) * radius;
        float x                   = center.x + r * std::cos(teta);
        float z                   = center.z + r * std::sin(teta);
        m_SmallSpherePositions[i] = float3{x, center.y - 1.0f, z};
    }

    // Lista de nombre de cubos pequenos
    m_SmallCubesNames.resize(MaxSmallCubes);
    for (int i = 0; i < MaxSmallCubes; ++i)
        m_SmallCubesNames[i] = "SmallCube" + std::to_string(i);

    // Lista de posiciones de cubos peque pequenas
    std::mt19937                          rng2{std::random_device{}()};

    m_SmallCubesPositions.resize(MaxSmallCubes);
    for (int i = 0; i < MaxSmallCubes; ++i)
    {
        float teta                = distAngle(rng2);
        float r                   = std::sqrt(distUnit(rng2)) * radius;
        float x                   = center.x + r * std::cos(teta);
        float z                   = center.z + r * std::sin(teta);
        m_SmallCubesPositions[i]  = float3{x, center.y - 1.0f, z};
    }
    ...
    // Setup camera.
    m_Camera.SetPos(float3(3.f, 10.5f, -30.0f));
    m_Camera.SetRotation(0.0f, -0.545f);
...
void Tutorial21_RayTracing::UpdateUI()
{
    ...
    if (ImGui::Begin("Settings", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        ImGui::Separator();
        ImGui::Text("Esferas pequeñas");
        ImGui::SliderInt("Number of small spheres", &m_shownSpheres, 0, 200);

        ImGui::Separator();
        ImGui::Text("Cubos pequenos");
        ImGui::SliderInt("Number of small cubes", &m_shownCubes, 0, 200);
